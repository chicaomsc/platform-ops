...
# conjunto, nunca só por tag. Semanticamente, um operador pensa "reverter para
# 1.2.0"; tecnicamente, o script só aceita reverter para um par tag+digest
# explícito (ou o baseline capturado automaticamente, que já contém ambos) —
# nunca "a tag que hoje se chama 1.2.0" sem confirmar que o digest bate com o
# que foi de fato registrado como bom anteriormente.
#
...
#      antes do deploy mais recente). Para reverter a uma versão mais antiga
#      que essa, as seis flags --to-*-tag/--to-*-digest são obrigatórias em
#      conjunto — o digest normalmente vem do histórico de commits de
#      release.yml (`git show <commit>:apps/<app>/<env>/release.yml`) ou de
#      versions.env legado. --to-release é apenas um rótulo para log/clareza
#      e exige as seis flags junto — o script não faz lookup automático de
#      digest histórico nesta sprint.
#
...
if [[ "$ANY_OVERRIDE" -eq 1 ]]; then
  if [[ -z "$TO_BACKEND_TAG" || -z "$TO_BACKEND_DIGEST" || -z "$TO_FRONTEND_TAG" || -z "$TO_FRONTEND_DIGEST" || -z "$TO_CADDY_TAG" || -z "$TO_CADDY_DIGEST" ]]; then
    die "as seis flags --to-*-tag/--to-*-digest devem ser passadas juntas (rollback exige tag E digest para cada componente — ver docs/versioning.md)" 10
  fi
  log "Versões-alvo fornecidas explicitamente: backend=${TO_BACKEND_TAG}@${TO_BACKEND_DIGEST} frontend=${TO_FRONTEND_TAG}@${TO_FRONTEND_DIGEST} caddy=${TO_CADDY_TAG}@${TO_CADDY_DIGEST}"
else
  ...
  TO_BACKEND_TAG="$(printf '%s\n' "$STATE_CONTENT" | grep '^BACKEND_TAG=' | cut -d= -f2-)"
  TO_BACKEND_DIGEST="$(printf '%s\n' "$STATE_CONTENT" | grep '^BACKEND_DIGEST=' | cut -d= -f2-)"
  TO_FRONTEND_TAG="$(printf '%s\n' "$STATE_CONTENT" | grep '^FRONTEND_TAG=' | cut -d= -f2-)"
  TO_FRONTEND_DIGEST="$(printf '%s\n' "$STATE_CONTENT" | grep '^FRONTEND_DIGEST=' | cut -d= -f2-)"
  TO_CADDY_TAG="$(printf '%s\n' "$STATE_CONTENT" | grep '^CADDY_TAG=' | cut -d= -f2-)"
  TO_CADDY_DIGEST="$(printf '%s\n' "$STATE_CONTENT" | grep '^CADDY_DIGEST=' | cut -d= -f2-)"
